...
class MainActivityPresenter : BasePresenter<MainActivityView> {
    ...
    fun searchMovieCoroutine(text: String, page: Int) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            val searchResults = MovieServiceBuilder().getMoviesCoroutine(Urls.MOVIEDB_API_KEY_VALUE, text, page).await()
            Log.e(TAG, "onNext" + searchResults.results?.get(0)?.originalTitle)

            withContext(Dispatchers.Main) {
                mView?.hideLoading()
                mView?.updateMovieAdapter(searchResults)
                val nextButtonGone = searchResults.totalPages!! < 2 ||
                        searchResults.page == searchResults.totalPages
                mView?.setUpOnNextPageButton(text,
                        if (nextButtonGone) View.GONE else View.VISIBLE, searchResults.page!! + 1)
